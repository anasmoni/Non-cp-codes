...
vector<double>Best4TourCost_NNH(4) , Best4TourCost_SH(4);
...
void TwoOptBestImp_NNH(vector<pdd>&v){
    ...
    for(int t=0; t<4; t++){
        ...
        for(int i=0; i<Best4Tour_NNH[t].size(); i++){

            for(int j = i+2; j<Best4Tour_NNH[t].size(); j++){

                double di = Dist(v[Best4Tour_NNH[t][i]], v[Best4Tour_NNH[t][i+1]]);
                double dj = Dist(v[Best4Tour_NNH[t][j]], v[Best4Tour_NNH[t][j+1]]);

                double dij = Dist(v[Best4Tour_NNH[t][i]], v[Best4Tour_NNH[t][j]]);
                double dij_ = Dist(v[Best4Tour_NNH[t][i+1]], v[Best4Tour_NNH[t][j+1]]);

                mn = min(mn , dij + dij_ - di - dj);
            }
        }

        avg += Best4TourCost_NNH[t] + mn;
        best = min(best , Best4TourCost_NNH[t] + mn);
        worst = max(best , Best4TourCost_NNH[t] + mn);
    }
