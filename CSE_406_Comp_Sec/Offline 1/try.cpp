...
bool isPossible(char c, char X)
{
    ...
    if(idx==mfc.size())
        return true; /// c is not in max freq list...nothing to check
    return false;
}
...
void solve(int ith_Word)
{
    ...
    for(int i=0,j,ii; (i+words[ith_Word].size()-1)<cipher.size(); i++)
    {
        ...
        for(j=0,ii=i; j<words[ith_Word].size(); j++,ii++)
        {
            ...
            else if(current_map[a] != X)
                break;
        }
        if(j == words[ith_Word].size())
            solve(ith_Word+1);
        current_map = prev;
    }
}
...
int main()
{
    ...
    while(iter--)
    {
        ...
        for(auto it : mapping) Xtoa[it.se] = it.fi;

        cout<<"Plain Text :   ";
        string pt;
        for(char c : cipher)
        {
            if(Xtoa.find(c) != Xtoa.end())
                pt += Xtoa[c];
            else
                pt += c;
        }
