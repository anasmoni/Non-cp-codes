...
void Draw_1_8_OfSphere(double radius,int slices,int stacks)
{
...
void DrawSphere()
{
	double stacks = 50 , slices = 50 , d = side , r = side_max - side ;

	glPushMatrix();
	{
		glTranslatef(d, d, d);
		Draw_1_8_OfSphere(r, slices, stacks);
	}
	glPopMatrix();
	...
	glPushMatrix();
	{
		glTranslatef(-d, -d, d);
		glRotated(180, 0, 0, 1);
		Draw_1_8_OfSphere(r, slices, stacks);  //x -ve, y -ve, z +ve
	}
	glPopMatrix();
...
{
	double d = side , r = side_max - side;

	glTranslatef(0, 0,side+r);
