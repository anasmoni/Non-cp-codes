...
void Delete_consecutivs(){
    struct listNode *temp = list , *dummy=NULL;

    while(temp){
        int tm = temp->item;

        while(temp->next){
            if(temp->next->item==tm)temp->next=temp->next->next;
            else break;
        }
        temp=temp->next;
    }
...
int main(void)
{
    ...
    while(1)
    {
        ...
        printf("4. InsertLast. 5. Print. 6. exit. 10. Delete_cons. \n");
        ...
        else if(ch==10)Delete_consecutivs();
